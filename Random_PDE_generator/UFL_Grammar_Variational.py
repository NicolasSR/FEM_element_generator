...
from Random_PDE_generator.latex_parser import LatexParser, conversions_list, parenthesis_table, categories_map
...
alphabet_with_greek = [
    ...
    ":/:Gamma", ":/:Delta", ":/:Eta", 
    ":/:Theta", ":/:Lambda", ":/:Xi", 
    ":/:Pi", ":/:Sigma", ":/:Upsilon", ":/:Phi", ":/:Chi", ":/:Psi", ":/:Omega"
]
...
class OperatorsTree():
    ...
    def add_new_child_node_as_user_function(self, parent_id, new_node_rank, child_slot):
        ...
        new_node.set_parent(parent_id)
        self.node(parent_id).add_child_id_in_slot(node_id, child_slot=child_slot)
        if num_arguments == 1:
...
class RandomEquationGenerator():
    ...
    def assign_variables(self, tree):
        ...
        symbol_candidates = self.symbols_list.copy()

        for rank_vars in all_variables.values():
            total_elements = len(rank_vars)
            if total_elements == 0:
                groups = None
            elif total_elements == 1:
                # groups = [0]
                symbol, symbol_id = Utilities.get_random_element(symbol_candidates)
                rank_vars[0].append(symbol)
                symbol_candidates.pop(symbol_id)
            else:
                ...
                for size in sizes:
                    ...
                    for element in elements[start_index:start_index + size]:
                        rank_vars[element].append(symbol)
                    start_index += size
        ...
        for raw_id in tree.upwards_traversal_order:
            if raw_id%1==0:
                ...
                for slot, child_id in current_node.children_ids.items():
                    if child_id is None:
                        current_symbol = variables_symbols[str([node_id,slot])]
                        ...
                        if parenthesis_table[categories_map[0][incoming_notation_type],categories_map[1][current_notation_type]] == 1:
                            repl = '('+repl+')' #TODO Add \left and \right randomly, or change by [] or {}
        ...
        def get_local_string(current_id):
            ...
            for slot, child_id in current_node.children_ids.items():
                if child_id is not None:
                    ...
                    current_string = re.sub(local_placeholders[slot], variables_symbols[str([current_id,slot])], current_string)
...
if __name__=="__main__":
    ...
    my_generator.convert_to_latex(tree,variables_symbols, chosen_conversions, parenthesis_table, categories_map)
    my_generator.convert_to_string(tree, variables_symbols)
